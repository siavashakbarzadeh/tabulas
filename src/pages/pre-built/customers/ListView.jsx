            ...
            <>
                ...
                <Menu.Items modal={false} ref={setDropdownContent} style={styles.popper} {...attributes.popper} className="absolute border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 rounded-md shadow z-[1000] min-w-[180px]">
                    <ul className="py-2">
                        ...
                        <li>
                            <Menu.Item as="button" className="w-full relative px-5 py-2.5 flex items-center rounded-[inherit] text-xs leading-5 font-medium text-slate-600 dark:text-slate-300 hover:text-primary-600 hover:bg-slate-50 hover:dark:bg-gray-900 transition-all duration-300">
                                <Icon className="text-start text-lg leading-none w-7 opacity-80" name="cart" />
                                <span>Orders</span>
                            </Menu.Item>
                        </li>
                        <li>
...
const CustomersListPage = () => {
  ...
  return (
    <>
        ...
        <PageHead>
            <PageHead.Group>
                <PageHead.Title>Customers List</PageHead.Title>
                <PageHead.SubTitle>You have total 1,261 customers.</PageHead.SubTitle>
            </PageHead.Group>
            <PageHead.Group>
                <PageHead.Option>
            ...
            <table className="border-collapse w-full border-gray-300 dark:border-gray-900"> 
                <thead>
                    <tr>
                        ...
                        <th className="py-2 px-2 first:ps-6 last:pe-6 border-b border-t border-gray-300 dark:border-gray-900 text-start hidden md:table-cell">
                            <span className="block text-sm leading-relaxed text-slate-400 font-normal">Payment Methods</span>
                        </th>
                        <th className="py-2 px-2 first:ps-6 last:pe-6 border-b border-t border-gray-300 dark:border-gray-900 text-start hidden 2xl:table-cell">
                            <span className="block text-sm leading-relaxed text-slate-400 font-normal">Joined</span>
                        </th>
                        <th className="py-2 px-2 first:ps-6 last:pe-6 border-b border-t border-gray-300 dark:border-gray-900 text-start hidden lg:table-cell">
                        ...
                        <th className="py-2 px-2 first:ps-6 last:pe-6 border-b border-t border-gray-300 dark:border-gray-900 text-end max-w-[3.75rem]">
                            <span className="block text-sm leading-relaxed text-slate-400 font-normal">Actions</span>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    return(
                        <tr key={index} className="transition-all duration-300 hover:bg-gray-50 hover:dark:bg-gray-1000 group">
                            ...
                            <td className="py-4 px-2 first:ps-6 last:pe-6 border-b border-gray-300 dark:border-gray-900">
                                <Link to={`/customer-details/${item.id}`} className="flex items-center">
                                    {item.img ? <Avatar size="sm" rounded img={item.img}/> : <Avatar variant={item.theme} size="sm" rounded text={toInitials(item.name)}/>}
                                    <span className="ms-3 block text-xs font-medium leading-6 text-slate-700 dark:text-white">
                                        {item.name}
                                        {item.status === "Active" && <span className="h-2 w-2 inline-block rounded-full bg-green-600 lg:hidden ms-1"></span>}
                                        {item.status === "Inactive" && <span className="h-2 w-2 inline-block rounded-full bg-yellow-600 lg:hidden ms-1"></span>}
                                        {item.status === "Suspended" && <span className="h-2 w-2 inline-block rounded-full bg-red-600 lg:hidden ms-1"></span>}
                                    </span>
                                </Link>
                            </td>
                            ...
                            <td className="py-4 px-2 first:ps-6 last:pe-6 border-b border-gray-300 dark:border-gray-900 hidden md:table-cell">
                                <div className="flex items-center">
                                    {item.cardType === "visa" && <Icon className="text-primary-500 text-xl w-7" name="visa" />}
                                    {item.cardType === "mastercard" && <Icon className="text-red-500 text-xl w-7" name="mc" />}
                                    {item.cardType === "amex" && <Icon className="text-indigo-800 text-xl w-7" name="american-express" />}
                                    <span className="text-sm text-slate-400">{item.cardNumber}</span>
                                </div>
                            </td>
                            <td className="py-4 px-2 first:ps-6 last:pe-6 border-b border-gray-300 dark:border-gray-900 hidden 2xl:table-cell">
                                <span className="text-sm text-slate-400">{item.joined}</span>
                            </td>
                            <td className="py-4 px-2 first:ps-6 last:pe-6 border-b border-gray-300 dark:border-gray-900 hidden lg:table-cell">
                                {item.status === "Active" && <span className="text-sm font-medium text-green-600">Active</span>}
                                {item.status === "Inactive" && <span className="text-sm font-medium text-yellow-600">Inactive</span>}
                                {item.status === "Suspended" && <span className="text-sm font-medium text-red-600">Suspended</span>}
                            </td>
                            <td className="py-4 px-2 first:ps-6 last:pe-6 border-b border-gray-300 dark:border-gray-900 text-end max-w-[3.75rem]">
                                <ul className="relative flex items-center justify-end -me-2">
                                    <li className="opacity-0 transition-all duration-300 bg-gray-50 dark:bg-gray-1000 px-0.5 group-hover:opacity-100">
                                        <Tooltip placement="top" content="Details">
                                            <Button.Zoom>
